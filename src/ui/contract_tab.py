...
class ContractTab(QWidget):
    ...
    def setup_ui(self):
        ...
        template_layout.addWidget(QLabel("<b>模板</b>"))
        self.template_combo = QComboBox()
        self.template_combo.setMinimumHeight(24)
        self.template_combo.setMinimumWidth(180)
        template_layout.addWidget(self.template_combo)
        options_layout.addWidget(template_widget)
    ...
    def generate_contract(self):
        ...
        # 模板选择
        contract.template_id = self.template_combo.currentData()
    ...
    def refresh_template_options(self):
        doc_type = self.doc_type_group.checkedId()
        options = self.document_service.list_templates(doc_type)

        if doc_type == 0:
            saved = self.settings.value("template_contract", "builtin", type=str)
        else:
            saved = self.settings.value("template_quote", "builtin", type=str)
        ...
        try:
            ...
            idx = self.template_combo.findData(saved)
            if idx >= 0:
    ...
    def save_settings(self):
        ...
        if self.doc_type_group.checkedId() == 0:
            self.settings.setValue("template_contract", template_id)
        else:
            self.settings.setValue("template_quote", template_id)
