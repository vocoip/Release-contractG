...
from src.utils.runtime_paths import get_resource_base_dir, ensure_writable_layout
...
class TemplateManagerDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        top_row.addWidget(self.doc_type_contract)
        top_row.addWidget(self.doc_type_quote)
        top_row.addStretch()
        card_layout.addLayout(top_row)

        list_row = QHBoxLayout()
        list_row.addWidget(QLabel("当前模板:"))
        self.template_combo = QComboBox()
        self.template_combo.setMinimumWidth(320)
        list_row.addWidget(self.template_combo)
        self.set_default_btn = QPushButton("设为默认")
        self.set_default_btn.clicked.connect(self.set_selected_as_default)
        list_row.addWidget(self.set_default_btn)
        list_row.addStretch()
        ...
        self.export_btn = QPushButton("导出内置默认")
        self.export_btn.clicked.connect(self.export_builtin_templates)
        self.open_folder_btn = QPushButton("打开模板目录")
        self.open_folder_btn.clicked.connect(self.open_templates_folder)
        ...
        self.doc_type_group.buttonClicked.connect(lambda _btn: self.refresh_templates())
        self.template_combo.currentIndexChanged.connect(lambda _idx: self._update_delete_state())

    def _doc_type(self):
        doc_type_id = self.doc_type_group.checkedId()
        return doc_type_id, ("contract" if doc_type_id == 0 else "quote")

    def refresh_templates(self):
        doc_type_id, _doc_type = self._doc_type()
        options = self.document_service.list_templates(doc_type_id)

        self.template_combo.blockSignals(True)
        try:
            self.template_combo.clear()
            for template_id, template_name in options:
    ...
    def _update_delete_state(self):
        ...
        self.delete_btn.setEnabled(bool(current_id and current_id != "builtin"))
    ...
    def export_builtin_templates(self):
        try:
            contract_xlsx, quote_xlsx = self.document_service.export_builtin_default_templates()
            QMessageBox.information(
                self,
                "导出成功",
                "已导出内置默认版式到 templates 目录：\n"
                f"- 合同：{os.path.basename(contract_xlsx)}\n"
                f"- 报价：{os.path.basename(quote_xlsx)}",
            )
            self.open_templates_folder()
        except Exception as e:
            QMessageBox.warning(self, "导出失败", f"导出内置默认模板时出错：{str(e)}")

    def set_selected_as_default(self):
        doc_type_id, doc_type = self._doc_type()
        template_id = self.template_combo.currentData()
        if not template_id or template_id == "builtin":
            QMessageBox.warning(self, "设置失败", "请选择一个非“内置默认”的模板")
            return

        key = "contract_template_default" if doc_type == "contract" else "quote_template_default"
        if not self.config_manager.set_setting("Templates", key, str(template_id)):
            QMessageBox.warning(self, "设置失败", "写入配置失败，请检查 settings.ini 是否可写")
            return

        QMessageBox.information(self, "成功", "已设置为默认模板")
        self.refresh_templates()
    ...
    def import_template(self):
        ...
        if src_path.suffix.lower() != ".xlsx":
            QMessageBox.warning(self, "导入失败", "请选择 .xlsx 格式的模板文件")
            return

        name_dialog = QDialog(self)
        name_dialog.setWindowTitle("导入模板")
        name_dialog.setMinimumWidth(420)
        layout = QVBoxLayout(name_dialog)
        form = QFormLayout()

        name_edit = QLineEdit()
        name_edit.setText(src_path.stem)
        form.addRow("模板显示名称:", name_edit)
        set_default_checkbox = QCheckBox("导入后设为默认模板")
        form.addRow("", set_default_checkbox)
        ...
        if not display_name:
            QMessageBox.warning(self, "导入失败", "模板名称不能为空")
            return

        existing = self.document_service.list_templates(doc_type_id)
        existing_ids = {template_id for template_id, _ in existing}
        existing_names = {template_name for _, template_name in existing}

        unique_name = display_name
        suffix = 2
        while unique_name in existing_ids or unique_name in existing_names:
        ...
        while dest_path.exists():
            dest_name = f"{src_path.stem}_{idx}{src_path.suffix}"
            dest_path = templates_dir / dest_name
            idx += 1
        ...
        try:
            ...
            QMessageBox.warning(self, "导入失败", f"复制模板文件失败：{str(e)}")
            return

        rel_path = f"templates/{dest_name}"
        if not self.config_manager.add_template(doc_type, unique_name, rel_path):
            QMessageBox.warning(self, "导入失败", "写入配置失败，请检查 settings.ini 是否可写")
            return

        if set_default_checkbox.isChecked():
            key = "contract_template_default" if doc_type == "contract" else "quote_template_default"
            self.config_manager.set_setting("Templates", key, unique_name)

        self.refresh_templates()
        new_idx = self.template_combo.findData(unique_name)
        if new_idx >= 0:
            self.template_combo.setCurrentIndex(new_idx)

        QMessageBox.information(self, "导入成功", f"模板已导入：{unique_name}\n文件：{dest_name}")
    ...
    def delete_selected_template(self):
        ...
        reply = QMessageBox.question(
            self,
            "确认删除",
            f"确定删除模板“{self.template_combo.currentText()}”吗？\n\n将从系统列表移除；若模板文件位于 templates 目录且未被其他模板引用，会同时删除该文件。",
            QMessageBox.Yes | QMessageBox.No,
            QMessageBox.No,
        )
        if reply != QMessageBox.Yes:
        ...
        ok, removed = self.config_manager.remove_template(doc_type, str(template_id))
        if not ok:
            QMessageBox.warning(self, "删除失败", "未找到该模板或写入配置失败")
            return

        removed_path = (removed or {}).get("path") or ""
        still_referenced = bool((removed or {}).get("still_referenced"))
        base_dir = ensure_writable_layout()
        templates_dir = (base_dir / "templates").resolve()
        removed_file = None
        if removed_path:
            removed_file = (base_dir / removed_path).resolve()
            if not str(removed_file).startswith(str(templates_dir) + os.sep):
                removed_file = None
            if removed_file and not removed_file.exists():
                removed_file = None

        if removed_file and not still_referenced:
            try:
                os.remove(str(removed_file))
            except Exception:
                pass
...
class MainWindow(QMainWindow):
    ...
    def create_menus(self):
        ...
        template_config_action = QAction("模板管理", self)
        template_config_action.triggered.connect(self.show_template_manager)
        edit_menu.addAction(template_config_action)
    ...
    def show_template_manager(self):
        dialog = TemplateManagerDialog(self.config_manager, self)
        dialog.exec_()
        self.contract_tab.refresh_template_options()
