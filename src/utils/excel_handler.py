...
from src.utils.runtime_paths import ensure_writable_layout, get_resource_base_dir
...
class ExcelHandler:
    """Excel处理工具类"""
    def __init__(self):
        self.base_dir = ensure_writable_layout()
        self.config_manager = ConfigManager()

        # 确保输出目录存在
        output_dir = self.config_manager.get_setting('General', 'output_dir', 'output') or 'output'
        self.output_dir = str((self.base_dir / output_dir).resolve())
    ...
    def _resolve_path_from_config(self, raw_path: str) -> str:
        ...
        resource_candidate = (get_resource_base_dir() / p).resolve()
        return str(resource_candidate)

    def _parse_template_list(self, raw: str):
        if not raw:
            return []
        ...
        for part in parts:
            ...
            resolved_path = self._resolve_path_from_config(path_part)
            base_name = name or Path(path_part).stem or "template"
            template_id = base_name
            suffix = 2
            while template_id in seen_ids:
                template_id = f"{base_name}_{suffix}"
                suffix += 1
    ...
    def _get_template_specs(self, doc_type: str):
        if doc_type == "contract":
            raw = (
                self.config_manager.get_setting("Templates", "contract_templates", None)
                or self.config_manager.get_setting("General", "contract_templates", None)
                or self.config_manager.get_setting("General", "contract_template", None)
            )
        elif doc_type == "quote":
            raw = (
                self.config_manager.get_setting("Templates", "quote_templates", None)
                or self.config_manager.get_setting("General", "quote_templates", None)
                or self.config_manager.get_setting("General", "quote_template", None)
            )
        else:
            return []

        specs = [s for s in self._parse_template_list(raw) if s.get("exists")]
        return specs

    def list_template_options(self, doc_type: str):
        options = [("builtin", "内置默认")]
        for spec in self._get_template_specs(doc_type):
            options.append((spec["id"], spec["name"]))
        return options
    ...
    def export_builtin_default_templates(self):
        ...
        contract = Contract(
            number="SC-20260101-001",
            customer=customer,
            company=company,
            items=items,
            sign_date="2026年01月01日",
            delivery_date="款到 14 天内",
            payment_method="款到发货",
            total_amount=2000.0,
            service_fee=1500.0,
            grand_total=3500.0,
        ...
        contract.random_suffix = "000"
        contract.template_id = "builtin"
        contract.convert_to_pdf = False
        contract.add_seal = False
        contract.convert_to_image_pdf = False

        original_output_dir = self.output_dir
        self.output_dir = str(templates_dir.resolve())
        try:
            contract_xlsx = self._generate_contract_file(contract)
            quote_xlsx = self._generate_quote_file(contract)
        finally:
    ...
    def _select_template_path(self, doc_type: str, template_id: str | None):
        ...
        specs = self._get_template_specs(doc_type)
        if not specs:
            return None

        if template_id:
            for spec in specs:
                if spec["id"] == template_id or spec["name"] == template_id:
                    return spec["path"]

        if doc_type == "contract":
            default_id = (
                self.config_manager.get_setting("Templates", "contract_template_default", None)
                or self.config_manager.get_setting("General", "contract_template_default", None)
            )
        else:
            default_id = (
                self.config_manager.get_setting("Templates", "quote_template_default", None)
                or self.config_manager.get_setting("General", "quote_template_default", None)
            )
        if default_id:
            for spec in specs:
                if spec["id"] == default_id or spec["name"] == default_id:
                    return spec["path"]

        return specs[0]["path"]

    def _build_template_context(self, contract, doc_type: str):
        company = getattr(contract, "company", None) or {}
        ...
        contract_number_base = getattr(contract, "number", "") or ""
        random_suffix = getattr(contract, "random_suffix", "") or ""
        contract_number_full = f"{contract_number_base}{random_suffix}" if random_suffix else contract_number_base

        quote_number = ""
        if contract_number_base and len(contract_number_base) >= 3:
            try:
                quote_number = f"BJ{contract_number_base[2:]}{random_suffix}"
            except Exception:
                quote_number = f"BJ{contract_number_base}{random_suffix}"

        delivery_value = getattr(contract, "delivery_date", "") or ""
        delivery_clause = f"三、交货时间：{delivery_value}。" if delivery_value else ""
        ...
        total_amount = money(getattr(contract, "total_amount", 0))
        service_fee_enabled = bool(getattr(contract, "service_fee_enabled", True))
        service_fee = money(getattr(contract, "service_fee", 0)) if service_fee_enabled else 0.0
        grand_total = money(getattr(contract, "grand_total", 0))
        ...
        ctx = {
            "合同编号": contract_number_base,
            "合同编号完整": contract_number_full,
            "报价单编号": quote_number,
            "签订日期": getattr(contract, "sign_date", "") or "",
            "交货时间": delivery_value,
            "交货条款": delivery_clause,
            "付款方式": getattr(contract, "payment_method", "") or "",
            "报价有效期": getattr(contract, "quote_valid_days", "") or "",
            "合同总金额": total_amount,
            "技术服务费": service_fee,
            "总计": grand_total,
            "合计": grand_total,
            "合计金额": grand_total,
            "甲方名称": safe_attr(customer, "name"),
            "甲方联系人": safe_attr(customer, "contact"),
            "甲方电话": safe_attr(customer, "phone"),
            "甲方地址": safe_attr(customer, "address"),
            "甲方开户银行": safe_attr(customer, "bank_name"),
            "甲方银行账号": safe_attr(customer, "bank_account"),
            "甲方税号": safe_attr(customer, "tax_id"),
            "乙方名称": company.get("name", "") or "",
            "乙方联系人": company.get("contact", "") or "",
            "乙方电话": company.get("phone", "") or "",
            "乙方地址": company.get("address", "") or "",
            "乙方开户银行": company.get("bank_name", "") or "",
            "乙方银行账号": company.get("bank_account", "") or "",
            "乙方税号": company.get("tax_id", "") or "",
        }
    ...
    def _replace_placeholders_in_cell(self, cell, ctx: dict):
        value = cell.value
        ...
        if value.strip() == f"{{{{{tokens[0]}}}}}" and len(tokens) == 1:
            key = tokens[0].strip()
            if key in ctx:
                cell.value = ctx[key]
            return
        ...
        for token in tokens:
            ...
            if key not in ctx:
                continue
            repl = ctx[key]
            new_value = new_value.replace(f"{{{{{token}}}}}", str(repl))
            new_value = new_value.replace(f"{{{{{key}}}}}", str(repl))
        cell.value = new_value

    def _replace_placeholders_in_sheet(self, ws, ctx: dict):
        for row in ws.iter_rows():
            for cell in row:
                self._replace_placeholders_in_cell(cell, ctx)

    def _expand_items_table(self, ws, items) -> bool:
        item_keys = {
            "序号",
            "商品名称",
            "规格型号",
        ...
        for row in ws.iter_rows():
            ...
            for cell in row:
                ...
                if raw in ("{{ITEMS_START}}", "{{ITEMS}}") and marker_cell is None:
                    marker_cell = cell
                ...
                for token in tokens:
                    if token.strip() in item_keys:
                        score += 1

            if score:
                candidate_scores[row[0].row] = score

        if marker_cell is not None:
            template_row = marker_cell.row
            marker_col = marker_cell.column
        else:
            if not candidate_scores:
                return False
            template_row = max(candidate_scores.items(), key=lambda kv: kv[1])[0]
            marker_col = None
        ...
        for col in range(1, max_col + 1):
            ...
            value = src.value
            if isinstance(value, str) and value.strip() in ("{{ITEMS_START}}", "{{ITEMS}}"):
                value = None
            snapshot.append(
                {
                    "col": col,
        ...
        for m in ws.merged_cells.ranges:
            min_col, min_row, max_col_m, max_row = m.bounds
            if min_row == max_row == template_row:
                merge_templates.append((min_col, max_col_m))

        insertion_count = max(0, len(items) - 1)
        shifted_merges = []
        if insertion_count:
            for m in list(ws.merged_cells.ranges):
                min_c, min_r, max_c, max_r = m.bounds
                if min_r > template_row:
                    shifted_merges.append((min_c, min_r, max_c, max_r))
                    ws.unmerge_cells(str(m))

            ws.insert_rows(template_row + 1, insertion_count)

            for min_c, min_r, max_c, max_r in shifted_merges:
                ws.merge_cells(
                    start_row=min_r + insertion_count,
                    start_column=min_c,
                    end_row=max_r + insertion_count,
                    end_column=max_c,
                )
        ...
        for i, item in enumerate(items):
            ...
            if i > 0 and merge_templates:
                for min_c, max_c in merge_templates:
                    ws.merge_cells(
                        start_row=row_idx,
                        start_column=min_c,
                        end_row=row_idx,
            ...
            item_ctx = {
                "序号": i + 1,
                "商品名称": getattr(item, "name", "") or "",
                "规格型号": getattr(item, "model", "") or "",
                "单位": getattr(item, "unit", "") or "",
                "数量": getattr(item, "quantity", "") or "",
                "单价": getattr(item, "price", "") or "",
                "单价(元)": getattr(item, "price", "") or "",
                "金额": getattr(item, "amount", "") or "",
                "金额(元)": getattr(item, "amount", "") or "",
                "备注": getattr(item, "remark", "") or "",
                "INDEX": i + 1,
            ...
            for s in snapshot:
                dst = ws.cell(row=row_idx, column=s["col"])
                dst.value = s["value"]
                ...
                dst.hyperlink = s["hyperlink"]

                if isinstance(dst.value, str) and "{{" in dst.value and "}}" in dst.value:
                    self._replace_placeholders_in_cell(dst, item_ctx)

        if marker_col is not None:
            ws.cell(row=template_row, column=marker_col).value = None
        return True

    def _expand_items_table_by_headers(self, ws, items) -> bool:
        if not items:
            return False

        header_aliases = {
            "序号": {"序号", "编号", "No", "NO", "No."},
            "商品名称": {"商品名称", "品名", "产品名称", "名称", "Name"},
            "规格型号": {"规格型号", "型号", "规格", "Model"},
            "单位": {"单位", "Unit"},
            "数量": {"数量", "Qty", "QTY"},
            "单价": {"单价", "单价(元)", "Unit Price", "Price"},
            "金额": {"金额", "金额(元)", "Total", "Amount"},
            "备注": {"备注", "Remark", "Remarks"},
        }
        ...
        for row in ws.iter_rows():
            row_idx = row[0].row
            row_values = {cell.column: norm(cell.value) for cell in row if cell.value is not None}
            score = 0
            cols = {}
            for key, aliases in header_aliases.items():
                for col, val in row_values.items():
                    if val in aliases:
                        ...
                        score += 1
                        break
            if score >= 4 and ("商品名称" in cols or "规格型号" in cols):
                header_row = row_idx
                header_cols = cols
                break
        ...
        for i, item in enumerate(items):
            ...
            def set_if(col_key, value):
                col = header_cols.get(col_key)
                if not col:
                    return
                ws.cell(row=row_idx, column=col).value = value

            set_if("序号", i + 1)
            set_if("商品名称", getattr(item, "name", "") or "")
            set_if("规格型号", getattr(item, "model", "") or "")
            set_if("单位", getattr(item, "unit", "") or "")
            set_if("数量", getattr(item, "quantity", "") or "")
            set_if("单价", getattr(item, "price", "") or "")
            set_if("金额", getattr(item, "amount", "") or "")
            set_if("备注", getattr(item, "remark", "") or "")

        return True

    def _generate_excel_from_template(self, template_path: str, contract, doc_type: str) -> str:
        wb = load_workbook(template_path)

        ctx = self._build_template_context(contract, doc_type)
        for ws in wb.worksheets:
            self._replace_placeholders_in_sheet(ws, ctx)
            items = getattr(contract, "items", []) or []
            expanded = self._expand_items_table(ws, items)
            if not expanded:
                self._expand_items_table_by_headers(ws, items)

        draft_suffix = "_草稿" if getattr(contract, "is_draft", False) else ""
        if doc_type == "contract":
    ...
    def _get_seal_file(self, contract):
        ...
        if hasattr(contract, 'company') and contract.company:
            # 处理公司是字典的情况
            if isinstance(contract.company, dict) and 'seal_image' in contract.company:
                seal_file = contract.company.get('seal_image')
            # 处理公司是对象的情况
            elif hasattr(contract.company, 'seal_image'):
                seal_file = contract.company.seal_image
            else:
                return None
            ...
            if seal_file:
                # 如果只是文件名，添加完整路径
                if not os.path.isabs(seal_file) and not seal_file.startswith('resources'):
                    seal_file = os.path.join('resources', 'seals', seal_file)
    ...
    def _generate_contract_file(self, contract):
        """生成合同文件"""
        template_path = self._select_template_path("contract", getattr(contract, "template_id", None))
        if template_path:
            try:
                return self._generate_excel_from_template(template_path, contract, "contract")
            except Exception as e:
                self.log(f"使用合同模板生成失败，将回退到内置默认模板: {str(e)}")
    ...
    def _generate_quote_file(self, contract):
        """生成报价单文件"""
        template_path = self._select_template_path("quote", getattr(contract, "template_id", None))
        if template_path:
            try:
                return self._generate_excel_from_template(template_path, contract, "quote")
            except Exception as e:
                self.log(f"使用报价模板生成失败，将回退到内置默认模板: {str(e)}")
